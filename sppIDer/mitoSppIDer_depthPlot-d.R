...
bedData <- read.table(paste(outputPrefix, "_winAvgDepth-d.txt", sep=""), header=F, skip=2, col.names = c("Genome_Pos", "species", "chrom", "start",  "end", "meanValue", "relativeMean", "max", "median"))
...
covMeanFile <- paste(outputPrefix, "_mitoAvgDepthSummary-d.txt", sep="")
...
pdf(paste(outputPrefix, "_mitoSppIDerDepthPlot-d.pdf", sep=""), width=14, onefile = TRUE)
